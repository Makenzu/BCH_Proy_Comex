...
namespace BCH.Comex.Core.BL.XEGI.Modulos
{
    public static class MODGSYB
    {
        ...
        public static string dbcharSy(string valor)
        {
            ...
            //dbcharSy = "'" + s + "'";
            dbcharSy = s;
            return dbcharSy == null ? string.Empty : dbcharSy;
        }
        ...
        public static string dbTCamSy(object valor)
        {
            ...
            MODXDOC.TipCam = MigrationSupport.Utils.Format(valor, "#0.0000").ToDbl();
            dbTCamSy = (MODXDOC.TipCam.ToInt()).Str().TrimB();

            return dbTCamSy;
        }
        // Retorna el valor de un campo teniendo el resultado de la consulta.-
        public static object GetPosSy(int Posicion, string tipo, string Resultado)
        {
            //OJO CON ESTE METODO, ESTABA RETORNANDO SIEMPRE DOUBLE
            object GetPosSy = null;
            ...
            else if (tipo == "C")
            {
                ...
                GetPosSy = ResultQuery;
            }
            else if (tipo == "F")
            {
                f = MODGDOC.CopiarDeString(ResultQuery.TrimB(), " ", 1);
                if (MigrationSupport.Utils.Format(f, "dd/mm/yyyy") == "01/01/1900")//TODO ARKANO VALIDAR QUE ESTA RETORNANDO
                {
                    // Fecha nula.-
                    GetPosSy = null;
                }
            ...
            else if (tipo == "H")
            {
                ...
                GetPosSy = (f.Left(8));//TODO ARKANO VALIDAR QUE ESTA RETORNANDO
            }
            else if (tipo == "D")
            {
