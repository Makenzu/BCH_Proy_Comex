...
namespace BCH.Comex.Core.BL.XGGL.Forms
{
    public static class Frm_Swf0
    {
        ...
        public static IList<UI_Message> ValidarSwiftCompleto(DatosGlobales initObject, UnitOfWorkCext01 uow, UnitOfWorkSwift uowSwift, T_Swf swiftNuevo, T_CliSwf cliente, T_mt103 montos, IList<LineaMensajeSwift> lineasManuales)
        {
            ...
            if (mensajes.Count == 0)
            {
                ...
                {
                    if (!swiftNuevo.BenSwf.EsBanco.ToBool())
                    {
                ...
                if (swiftNuevo.BenSwf.EsBanco.ToBool() && !swiftNuevo.EsAladi.ToBool())
                {
                    lineasManuales = lineasManuales.Where(lm => lm.CodMT == T_MODGSWF.MT_202 && lm.Incluido == true).ToList();
                }
                else
                {
                    lineasManuales = lineasManuales.Where(lm => (lm.CodMT == T_MODGSWF.MT_103 || lm.CodMT == T_MODGSWF.MT_100) && lm.Incluido == true).ToList();
                }
        ...
        public static bool GenerarSwift(DatosGlobales initObject, UnitOfWorkCext01 uow, T_Swf swiftNuevo, T_CliSwf cliente, T_mt103 montos, IList<CodigoDeOrdenCampo23Swift> codigosCampo23, IList<LineaMensajeSwift> lineasManuales, short indiceSwift)
        {
            swiftNuevo.DatSwf.ctacte = string.IsNullOrEmpty(swiftNuevo.DatSwf.ctacte) ? string.Empty : swiftNuevo.DatSwf.ctacte.Trim();
            swiftNuevo.BenSwf.NomBen = string.IsNullOrEmpty(swiftNuevo.BenSwf.NomBen) ? string.Empty : swiftNuevo.BenSwf.NomBen.Trim();
            swiftNuevo.BenSwf.DirBen1 = string.IsNullOrEmpty(swiftNuevo.BenSwf.DirBen1) ? string.Empty : swiftNuevo.BenSwf.DirBen1.Trim();
            swiftNuevo.BenSwf.DirBen2 = string.IsNullOrEmpty(swiftNuevo.BenSwf.DirBen2) ? string.Empty : swiftNuevo.BenSwf.DirBen2.Trim();
            ...
            if (swiftNuevo.EsAladi.ToBool())
            {
                swiftNuevo.DatSwf.NroAla = Fn_Numero_Aladi(initObject, uow);
            ...
            else
            {
                ...
                if (banco != null)
                {
                    ...
                    swiftNuevo.DatSwf.CtaCor = banco.Nom_cta.Trim();
                }
            }
            ...
            initObject.MODGSWF.VSwf[indiceSwift].DocSwf = BCH.Comex.Core.BL.XCFT.Modulos.MODGSWF.GeneraDocSwf(initObject.MODGSWF, initObject.MODGUSR, initObject.MODGTAB0,
                null, null, initObject.MOD_50F, initObject.MESSAGES, null, false.ToShort(), uow, lineasManuales, indiceSwift);
        ...
        private static string Fn_Numero_Aladi(DatosGlobales initObj, UnitOfWorkCext01 uow)
        {
            double q = 0;
            string Agno = "";
            string Correlativo = "";
            ...
            q = MODGRNG.LeeSceRng(initObj, uow, T_MODGRNG.Rng_OpeAla);
            if (q == -1)
            {
            ...
            decimal? sucBCH = uow.SceRepository.sce_obc_s01_MS((short)initObj.MODGSCE.VGen.SucBCH);
