...
function BuscarDocsParaImprimir() {
    if (ValidateFilters(true)) {
        if (esPrimera) {
            ...
            tablaDocs.on("load-error.bs.table", BusquedaOnError);
...
window.operateEvents = {
    ...
    'click .verContabilidad': function (e, value, row, index) {
        ...
        paramsUltimaAccion.format = 'HTML';
        paramsUltimaAccion.filename = row['NroOperacion'].replace(/-/g, ''); //remuevo todos los guiones;
...
function GetParamsParaContabilidad(row) {
    ...
    return { nroReporte: row['NroRpt'], fechaOp: moment.utc(row['FechaOperacion']).format("YYYY/MM/DD") };
}

function GetParamsParaCarta(row) {
    var nroOperacion = row['NroOperacion'].replace(/-/g, ''); //remuevo todos los guiones
    return { numeroOperacion: nroOperacion, codDocumento: row['CodigoPropio'], nroCorrelativo: row['NroRpt'] };
}
...
function ImprimirDocumento(paramsAccion) {
    if (paramsAccion != null) {
        var location = "";
        if (paramsAccion['numeroOperacion'] != null) {
        ...
        else if (paramsAccion['nroMensaje'] != null) {
            location = urlVerDetalleMensajeSwift + "?nroMensaje=" + paramsAccion['nroMensaje'] +"&nroReporte=" + paramsAccion['nroReporte'] + "&fechaOp=" + encodeURIComponent(paramsAccion['fechaOp'])  + "&generarHtmlCompleto=true";
        }
        else if (paramsAccion['nroReporte'] != null) {
    ...
    if (selections.length === 0 || selections.length == 0) {
        ...
        if (ConfigImpres_PrintFormat == 'HTML') {
            ...
            for (i = 0; i < selections.length; ++i) {
                ...
                else if (filename != item['NroOperacion'].replace(/-/g, ''))
                    filename = "";
            }
