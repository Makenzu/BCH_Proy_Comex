...
namespace BCH.Comex.Common
{
    public class Printer
    {
        ...
        public void PrintList(params object[] input)
        {
            PrintWithoutCrlf(input);
            NewLineContent();
        }
        ...
        {
            StringBuilder output = new StringBuilder();
            foreach (var line in this.document)
            {
                output.AppendLine(String.Join("", line.Content.Select(w => w.Content)));
            }
            return output.ToString();
        }
        ...
        {
            short dif = (short)((tabInfo.Column - this.currentLine.Content.Sum(x => x.Content.Length) - this.currentContent.Length) - 1);
